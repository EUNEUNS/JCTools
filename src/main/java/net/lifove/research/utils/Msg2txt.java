...
public class Msg2txt {
	...
	public void processMessage() throws IOException {
		...
		try {
			...
			try {
				String[] displayString = msg.getHeaders();
				boolean exist = false;
				String fromString = "";
				String dateString = "";
				for(String a: displayString){
					
					//System.out.println(a);
					
					if(a.startsWith("Reply-To: "))
						fromString += a;
					
					if(fromString.equals("") && a.startsWith("From: ")){
						fromString += a;
					}
				...
				txtOut.println(fromString + "|" + dateString);
	...
	public static void main(String[] args) {
		if(args.length <= 0) {
			...
			// find all msg files in this directory and sub-directories.
			visitAllDirsAndFiles(new File("."));
		} else {
	...
	// Process all files and directories under dir
	 public static void visitAllDirsAndFiles(File dir) {
		 ...
		 try {
			 	...
			 	if(dir.getName().toLowerCase().endsWith("msg")){
					Msg2txt processor = new Msg2txt(dir.getPath());
					processor.processMessage();
			 	}
			} catch (IOException e) {
				System.err.println("Could not process "+dir+": "+e);
			}
